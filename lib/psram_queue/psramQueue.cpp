...
** \brief psramQueue handling library (designed on Arduino)
** \details psramQueue handling library (designed on Arduino)
...
*limit \b end is reached *	\param [in,out] pIdx - pointer to index value
...
inc_idx(uint16_t *const pIdx, const uint16_t end, const uint16_t start) {
  if (*pIdx < end - 1) {
    (*pIdx)++;
  } else {
    *pIdx = start;
...
*limit \b start is reached *	\param [in,out] pIdx - pointer to index value
**	\param [in] end - counter upper limit value
**	\param [in] start - counter lower limit value
**/
static inline void __attribute__((nonnull, always_inline))
dec_idx(uint16_t *const pIdx, const uint16_t end, const uint16_t start) {
  if (*pIdx > start) {
    (*pIdx)--;
  } else {
    *pIdx = end - 1;
...
cppQueue::cppQueue(const uint16_t size_rec, const uint16_t nb_recs,
                   const cppQueueType type, const bool overwrite) {
  ...
  if (queue == NULL) {
    queue_sz = 0;
    return;
  }  // Return here if cppQueue not allocated
  else {
    queue_sz = size;
  }
  ...
  else if (ovw)  // cppQueue is full and overwrite is allowed
  {
    if (impl == FIFO) {
      inc_idx(&out, rec_nb, 0);
       ...
       // else if (impl == LIFO)	{}								// Nothing to do in this
    // case
  }
