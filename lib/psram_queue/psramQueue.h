...
**			This library was designed for Arduino, yet may be
*compiled without change with gcc for other purposes/targets
**/
/****************************************************************/
#ifndef __CPPQUEUE_H
#define __CPPQUEUE_H
...
class cppQueue {
 private:
  ...
  uint16_t init;  //!< set to QUEUE_INITIALIZED after successful init of the
                  //!< queue, 0 otherwise
 public:
  ...
  **	\return nothing
  **/
  cppQueue(const uint16_t size_rec, const uint16_t nb_recs = 20,
           const cppQueueType type = FIFO, const bool overwrite = false);
  ...
  **	\deprecated clean was already used in cppQueue lib, alias is made to
  *keep compatibility with earlier versions
  **/
  inline void clean(void) __attribute__((always_inline)) { flush(); }
  ...
  **/
  inline uint16_t getCount(void) __attribute__((always_inline)) { return cnt; }

  /*!	\brief get number of records in the queue (same as getCount)
  **	\deprecated nbRecs was already used in cppQueue lib, alias is made to
  *keep compatibility with earlier versions *	\return Number of records stored
  *in the queue
  **/
  ...
  **	\warning If using push, pop, peek, drop, peekItem and/or peekPrevious in
  *both interrupts and main application,
  **				you shall disable interrupts in main application when
  *using these functions *	\param [in,out] record - pointer to record to be
  *popped from queue *	\return Pop status *	\retval true if successfully
  *popped from queue *	\retval false if queue is empty
  **/
  ...
  *both interrupts and main application,
  **				you shall disable interrupts in main application when
  *using these functions *	\deprecated pull was already used in cppQueue
  *lib, alias is made to keep compatibility with earlier versions *	\param
  ...
  *both interrupts and main application,
  **				you shall disable interrupts in main application when
  *using these functions *	\note This function is most likely to be used in
  *conjunction with drop *	\param [in,out] record - pointer to record to be
  *peeked from queue *	\return Peek status *	\retval true if successfully
  *peeked from queue *	\retval false if queue is empty
  **/
  ...
  *both interrupts and main application,
  **				you shall disable interrupts in main application when
  *using these functions *	\note This method is most likely to be used in
  *conjunction with peek *	\return drop status *	\retval true if
  *successfully dropped from queue *	\retval false if queue is empty
  **/
  bool drop(void);
  ...
  *both interrupts and main application,
  **				you shall disable interrupts in main application when
  *using these functions *	\note This function is only useful if searching
  *for a duplicate record and shouldn't be used in conjunction with drop
  ...
  *both interrupts and main application,
  **				you shall disable interrupts in main application when
  *using these functions *	\note This inline is only useful with FIFO
  *implementation, use peek instead with a LIFO (will lead to the same result)
  **	\param [in,out] record - pointer to record to be peeked from queue
  **	\return Peek status
  **	\retval true if successfully peeked from queue
  **	\retval false if queue is empty
  **/
  inline bool peekPrevious(void* const record)
      __attribute__((nonnull, always_inline)) {
    const uint16_t idx = getCount() - 1;  // No worry about count - 1 when queue
                                          // is empty, test is done by peekIdx
